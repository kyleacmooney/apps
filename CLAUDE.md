...
- **AI token storage** — `lib/token-storage.ts` abstracts Claude OAuth token storage across three backends: `local` (localStorage — fast, basic security), `shared` (shared Supabase `user_settings.claude_oauth_token` — RLS-protected, encrypted at rest), `private` (user's external Supabase `user_secrets.claude_oauth_token` — same protections on user-controlled infra). Storage mode preference is in localStorage (`claude-token-storage-mode`). Token reads are async (for server-backed modes) with in-memory caching. Switching modes auto-migrates the token. Settings UI shows available options based on auth state + external backend config.
...
- **`user_secrets`:** Optional table on user's external Supabase project for storing sensitive config like `claude_oauth_token` (when "Your Supabase" storage is chosen). Defined in `docs/schema.sql`; auth-gated policies in `docs/schema-auth.sql`.
- Edge Functions: `proxy-image-upload` (downloads external image URLs and self-hosts them in Supabase Storage, updating `species_profiles.image_url`), `chat` (proxies streaming Claude API calls; Chat page sends user's Supabase session + Claude OAuth token, edge validates session and forwards to Anthropic). **Deploy chat with** `supabase functions deploy chat --no-verify-jwt` — auth is verified inside the function; gateway JWT verification can reject valid session tokens.
- **AI Chat:** The in-app AI chat (`pages/Chat.tsx`) calls the `chat` edge function with the user's Supabase session (Authorization header) and Claude OAuth token in the body. The edge function validates the session and proxies the request to the Anthropic API so the call is server-side (avoids CORS org restrictions). Users configure their Claude token in Settings; it is sent only to the edge and then to Anthropic, not stored on Supabase.
...
- **Edge Functions** (`supabase/functions/<function-name>/`): Every deployed edge function must have its source checked into the repo. After deploying via `deploy_edge_function` MCP tool or `supabase functions deploy`, ensure the source files are committed. One directory per function, matching the function slug. The `chat` function must be deployed with `--no-verify-jwt` (e.g. `supabase functions deploy chat --no-verify-jwt`); auth is enforced inside the function.
