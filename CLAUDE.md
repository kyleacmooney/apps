...
- **AI token storage** — `lib/token-storage.ts` supports three Claude OAuth token backends: `local` (browser `localStorage`), `shared` (shared Supabase `user_settings.claude_oauth_token`), and `private` (user's external Supabase `user_secrets.claude_oauth_token`). `shared` is the safest supported mode because the chat edge function reads it server-side; the browser does not resend the token on each chat request. `private` is only available when the external project has Google auth enabled and auth-gated RLS on `user_secrets`. Storage mode preference is in localStorage (`claude-token-storage-mode`). Switching modes auto-migrates the token.
...
- **Auto-merge:** Pushes to `claude/*`, `cursor/*`, and `codex/*` branches are automatically fast-forward merged to `dev` (staging) and the branch is deleted. Workflow: `.github/workflows/auto-merge-claude.yml`. Changes land at `/apps/dev/` first — not production.
